...
Source scene + animation: dev/icon/build_scene.js -> dev/icon/enchanted_timber_icon_anim.bbmodel (Aseprite textures from dev/icon/draw_sprites.lua).
Background: dev/icon/sprites/bg_flat.aseprite (64px, flat colour + ground shadow placed for the current crop).
ICON_BG=<png> overrides the background (for option sheets)."""
import os
import sys
...
ICON = ROOT / "dev" / "icon"
BACKGROUND = Path(os.environ.get("ICON_BG", ICON / "sprites" / "bg_flat.png"))
DIST = ROOT / "dist"
S = 512
FPS = 25
STILL_FRAME = 10  # runes lifting off the book toward the axe
GIF_SIZE = 256
GIF_LIMIT = 256 * 1024
PALETTE = 255
CROP_PCT = 0.90
EXTRA_FRAMES = (29, 30, 31)  # the backswing and strike: brief, but must stay fully in frame
CROP_MARGIN = 1.12
OUTLINE = 19
PACK_MARGIN = 1.12
