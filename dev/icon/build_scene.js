// Enchanted Timber icon scene + animation (pack-icon-animation skill). Run inside Blockbench (free format project):
...
// Story: act 1 (Enchanted Veinminer's) - an enchanted book's glint lifts off as runes and flies into the axe, the book is
...
var VM = (function () {
  ...
  const Z_ITEMS = 40, Z_FX = 50, Z_AXE = 30, Z_DEBRIS = 35, Z_BOOK = 44, Z_RUNE = 48;
  ...
  // Timber's timing (act 2), shifted by SHIFT below
  const T = {
  ...
  for (const k of Object.keys(T)) T[k] = Array.isArray(T[k]) ? T[k].map(v => v + SHIFT) : T[k] + SHIFT;
  // act 1
  Object.assign(T, {
    bookGlint: 0.16,                                   // the book pops in at 0 (cheap: nothing else moves then)
    runes: [0.32, 0.32, 0.36, 0.40], runeFlight: 7,    // a tight burst: the GIF redraws only the cluster's box
    charge: 0.68, bookPoof: 0.44,
    axeGlints: [0.80, 2.84],                           // after the charge, and while the drops lie still (cheap)
  });
  const P = {
    book: [-48, 77], runeArc: 18, runeFan: 5,          // book floats in the empty sky the tree later falls into; runes fan over the crown
  ...
  // Blockbench is shared with other sessions: every entry point selects this project first (Texture.all, Outliner and
  // Animation act on the selected project), and render() holds window.BB_LOCK
  const PROJECT = 'enchanted_timber_icon', ME = 'enchanted-timber';
  function own() {
  ...
  function build() {
    ...
    AXE_VARIANTS.forEach(v => {
      const g = G['av_' + v] = group('av_' + v, [L.grip.x, L.grip.y, Z_AXE], G.axe);
      extrude('axe', v, AXE_GRIP, [L.grip.x, L.grip.y, Z_AXE], AXE_PX, g);
    });
    ...
    BOOK_VARIANTS.forEach((v, k) => {
      ...
      plane('book_plane', [L.book.x, L.book.y - (12 - BOOK_ART_CY) * BOOK_PX, Z_BOOK + k * 0.01], bs, g, v, 16);
    });
    G.bookPuff = group('bookPuff', [L.book.x, L.book.y, Z_FX + 6], G.screen);
    plane('book_puff_plane', [L.book.x, L.book.y, Z_FX + 6], 27, G.bookPuff, 'fx_puff', 16);
    G.bookSpark = group('bookSpark', [L.book.x + 8, L.book.y + 6, Z_FX + 6], G.screen);
    plane('book_spark_plane', [L.book.x + 8, L.book.y + 6, Z_FX + 6], 8, G.bookSpark, 'fx_spark_violet', 16);
    T.runes.forEach((t, r) => {
      const s = runeStart(L, r), k = r % 4, u = (k % 2) * 8, v = Math.floor(k / 2) * 8;
      G['rune_' + r] = group('rune_' + r, [s.x, s.y, Z_RUNE + r * 0.1], G.screen);
      cube('rune_plane', [s.x - 4 * RUNE_PX, s.y - 4 * RUNE_PX, Z_RUNE + r * 0.1], [s.x + 4 * RUNE_PX, s.y + 4 * RUNE_PX, Z_RUNE + r * 0.1],
        G['rune_' + r], { south: ['fx_rune', [u, v, u + 8, v + 8]] });
    });
    G.chargeRing = group('chargeRing', [L.head.x, L.head.y, Z_FX + 7], G.screen);
    plane('charge_ring_plane', [L.head.x, L.head.y, Z_FX + 7], 24, G.chargeRing, 'fx_ring_violet', 16);
    CHARGE_SPARKS.forEach((a, k) => {
      G['cspark_' + k] = group('cspark_' + k, [L.head.x, L.head.y, Z_FX + 9], G.screen);
      plane('cspark_plane', [L.head.x, L.head.y, Z_FX + 9], 7, G['cspark_' + k], 'fx_spark_violet', 16);
    });
  ...
  // ---- geometry ----
  const AXE_PX = 1.4, AXE_GRIP = [10.5, 19], AXE_TIP = [1, 6.5], AXE_HEAD = [6, 6.5], TRAIL_DIR = 80, SMEAR_R = 24;
  const AXE_VARIANTS = ['axe_item', 'axe_flash', 'axe_glint_0', 'axe_glint_1', 'axe_glint_2', 'axe_glint_3', 'axe_glint_4'];
  const BOOK_VARIANTS = ['book_open', 'book_glint_0', 'book_glint_1', 'book_glint_2', 'book_glint_3', 'book_glint_4'];
  const BOOK_PX = 0.95, BOOK_ART_CY = 9.5, RUNE_PX = 0.8;
  ...
  function layout() {
    ...
    // axe head centre in the rest pose (rotation restRot about the grip, plus the rest offset)
    const hOff = rot2([(AXE_HEAD[0] - AXE_GRIP[0]) * AXE_PX, (AXE_GRIP[1] - AXE_HEAD[1]) * AXE_PX], P.restRot);
    const head = { x: grip.x + P.rest[0] + hOff[0], y: grip.y + P.rest[1] + hOff[1] };
    ...
    return { tip, grip, head, book, apple, plings, puffs, dust, leafFrom, land, brk };
  }
  ...
  const z = r => [0, 0, r];
  // show exactly one child variant at a time: schedule = [[t, name], ...]
  ...
  function animate() {
    ...
    // ---- act 1: the book's glint flows into the axe ----
    variants('bv_', BOOK_VARIANTS, [[0, 'book_open'], ...sweep(T.bookGlint, 'book_glint_'), [T.bookGlint + 5 * DT, 'book_open']]);
    const bp = T.bookPoof, r0 = T.runes[0];
    track(G.book, 'scale', [
      [0, 0], [0.04, [0.8, 1.3, 1]], [0.08, [1.2, 0.85, 1]], [0.12, [0.94, 1.08, 1]], [0.16, [1.03, 0.98, 1]], [0.24, 1],
      [r0 - 0.04, 1], [r0, [1.08, 0.93, 1]], [r0 + 0.04, [0.96, 1.06, 1]], [r0 + 0.08, 1],
      [bp - 0.04, 1], [bp, [1.15, 0.85, 1]], [bp + 0.04, [0.8, 1.2, 1]], [bp + 0.08, 0], [LEN, 0],
    ]);
    const pf = bp + 0.08;
    ...
    track(G.bookSpark, 'scale', [[0, 0], [0.04, 0.7], [0.08, 1.3], [0.12, 0.8], [0.16, 0]]);
    track(G.bookSpark, 'rotation', [[0, z(0)], [0.16, z(45)]]);

    // runes: pop off the page, arc over to the axe head, shrink into it
    const arrivals = [];
    T.runes.forEach((te, r) => {
      const g = G['rune_' + r], s = runeStart(L, r), n = T.runeFlight;
      const dx = L.head.x - s.x, dy = L.head.y - s.y, arc = P.runeArc + r * P.runeFan;
    ...
    // axe variants: plain until charged, white flash on the charge, glint sweeps while enchanted
    const av = [[0, 'axe_item'], [tc, 'axe_flash'], [tc + 2 * DT, 'axe_item']];
    T.axeGlints.forEach(t => av.push(...sweep(t, 'axe_glint_'), [t + 5 * DT, 'axe_item']));
    variants('av_', AXE_VARIANTS, av.filter(([t]) => t < LEN - 1e-6));
    const bumps = [];
    if (arrivals[0] < tc - 0.04 - 1e-6) bumps.push([arrivals[0], [1.06, 1.06, 1]]);   // first runes land: small bump, then the charge pop
  ...
  function layoutInfo() {
    ...
    return JSON.stringify({ grip: [r(L.grip.x), r(L.grip.y)], head: [r(L.head.x), r(L.head.y)], book: [r(L.book.x), r(L.book.y)], tip: [r(L.tip.x), r(L.tip.y)], apple: [r(L.apple.x), r(L.apple.y)], puffs: L.puffs.map(p => [r(p.x), r(p.y)]) });
  }
  ...
  return { own, lockedBy, FPS, DT, LEN, T, P, PITCH, G, tex, loadTextures, build, animate, camera, render, setTime, scaleSweep, probe, layout, layoutInfo, pixels, worldToScreen, yawToWorld, yawToScreen, fallen, q };
})();
