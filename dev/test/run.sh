...
python3 "$BASE/dev/build.py" >/dev/null
...
cd "$WORK"
set +e
java -Xmx2G --add-opens java.base/java.lang=ALL-UNNAMED ${JAVA_OPTS:-} -cp "$WORK/classes:$JAR:$CP" EnchantedTimberTest "$@" 2>&1 | tee "$WORK/harness.log" | grep -oE '\[(PASS|FAIL|INFO|EXPLORE)\].*|SUMMARY.*|  - .*|[A-Za-z.]*Exception.*'
STATUS=${PIPESTATUS[0]}
set -e
