#!/usr/bin/env python3
"""Validate the pack and build dist/EnchantedTimber-<version>.zip (deterministic)."""
import json
import re
import sys
import zipfile
...
def build(ver: str) -> Path:
    ...
    out = DIST / f"EnchantedTimber-{ver}.zip"
    for old in DIST.glob("EnchantedTimber-*.zip"):
        old.unlink()
...
def main() -> None:
    ...
    for rel, needle in (("pack/pack.mcmeta", f"v{ver}"),
                        (f"pack/data/{NAMESPACE}/function/uninstall.mcfunction", f"EnchantedTimber-{ver}.zip"),
                        ("CHANGELOG.md", f"## {ver}")):
